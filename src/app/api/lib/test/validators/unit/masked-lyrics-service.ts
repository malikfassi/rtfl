export const maskedLyricsService = {
  create: (key: string, maskedLyrics: any) => {
    ...
    expect(maskedLyrics.title.some((t: any) => t.isToGuess)).toBe(true);
    expect(maskedLyrics.artist.some((t: any) => t.isToGuess)).toBe(true);
    expect(maskedLyrics.lyrics.some((t: any) => t.isToGuess)).toBe(true);
    // Optionally, check that all tokens have 'value' and 'isToGuess'
    for (const section of ['title', 'artist', 'lyrics']) {
      for (const token of maskedLyrics[section]) {
